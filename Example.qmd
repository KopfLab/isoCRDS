...
source("source/iso_map_crds.R")
source("source/iso_plot_crds.R")
source("source/iso_read_crds.R")
...
### Step 3, Option 1: Interactive Mapping

You may opt to use our interactive Shiny app for sample mapping. Run the Shiny app. You will be prompted to upload the `.RDS` file that you generated in step 2. Once you've uploaded the file, you can create an interactive plot.

1.  Select plot parameters that help you see where your samples are in the data.
2.  Use the rectangular select tool to select your sample data.
3.  Press the "Add Sample_ID" button. This marks that selected data as belonging to your sample.\
    For example, selecting the plateau a region of heightened $CO_2$ or $CH_4$ .
4.  When you are done selecting data and inputting sample names, use the Save Mapped Data button to save an updated `.RDS` file that is mapped.
